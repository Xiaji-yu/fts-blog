...
// Convert embeds ![[file]] and wiki links [[Link]] / [[Link|Alias]] on a
// single line. Extracted so table rows can run link conversion before pipe
// escaping (escaping first would corrupt the `|` alias separator).
function convertLinksAndEmbeds(line, slugToIdMap) {
  // Step 1: Embeds ![[file]] - must come before wiki links (avoid conflict)
...
function convertObsidianSyntax(content, slugToIdMap = {}) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Handle table rows - convert links first (removes `[[a|b]]` alias pipes),
    // then escape remaining pipes inside cell content.
    const isTableRow = line.trim().startsWith('|') && line.trim().endsWith('|');
    const isTableSeparator = /^\|?[\s\-:]+(\|[\s\-:]*)+\|?$/.test(line.trim());

    if (isTableRow && !isTableSeparator) {
      const converted = convertLinksAndEmbeds(line, slugToIdMap);
      result.push(escapeTablePipes(converted));
      continue;
    }

    result.push(convertLinksAndEmbeds(line, slugToIdMap));
  }
